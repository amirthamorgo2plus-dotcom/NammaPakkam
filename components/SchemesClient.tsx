...
import type { CmsScheme } from '@/lib/cms';

export default function SchemesClient({ schemes }: { schemes: CmsScheme[] }) {
  const { t } = useI18n();
  const national = schemes.filter((s) => s.scope === 'National');
  const tn = schemes.filter((s) => s.scope === 'Tamil Nadu');

  const Card = (s: CmsScheme) => (
    <div key={s.id} className="card p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          {s.who && <div className="text-[11px] font-medium text-brand-600 mt-0.5">For: {s.who}</div>}
          {s.description && <p className="text-sm text-stone-600 mt-1">{s.description}</p>}
          {s.link && (
            <a href={s.link} target="_blank" rel="noopener noreferrer" className="text-xs text-brand-600 font-medium mt-1 inline-block">
              Official site →
            </a>
          )}
        </div>
